...
import com.project.horror.community.repository.BoardMapper;
...
//        convertDateFormat(boardList);
    ...
    private void substringTitle(Board b) {
        ...
        if (title.length() > 15) {
            String subStr = title.substring(0, 15);
...
//        new Cookie("쿠키이름", "쿠키값"); // 쿠키 생성
    ...
    // 좋아요 상승 처리
    public boolean upLikeCountService(Board board) {
        log.info("upLikeCount service start - {}", board);
        return mapper.upLikeCount(board);
    }

    // 좋아요 이력 생성
    public boolean makeLikeHistoryService(Board board) {
        log.info("makeLikeHistory service start - {}", board);
        return mapper.makeLikeHistory(board);
    }

    // 좋아요 생성이력 확인
    public int checkLikeHistoryService(Board board) {
        log.info("checkLikeHistory service start - {}", board);
        return mapper.checkLikeHistory(board);
    }
}
